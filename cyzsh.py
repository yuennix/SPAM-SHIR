...
[›] PAID TOOL
                    ...
                    if 'cookie' in resource:
                        if not self.validate_cookie_format(resource['cookie']):
                            ...
                            if token:
                                ...
                                details = f"{len(test_result.get('pages', []))} pages" if test_result['valid'] else "Token fail"
                            else:
    ...
    async def bulk_extract(self):
        ...
        self.print_panel(
            "Bulk Extraction",
            "Paste cookies/tokens below (one per line)\n"
        ...
        if choice == "file":
            ...
            try:
                ...
                self.print_panel("Error", f"Failed to read file: {str(e)}", "red")
                input("\n[Press Enter to continue]")
                await self.show_token_generator()
                return
        else:
        ...
        self.clear_screen()
        self.show_banner()
        self.print_panel("Processing", f"Extracting {len(items)} items...", "blue")
                ...
                try:
                    ...
                    if is_cookie:
                        if self.validate_cookie_format(item):
                            token = self.get_token_from_cookie(item)
                            if token:
                                test_result = await self.verify_token(token)
                                if test_result['valid']:
                                    results['valid_cookies'].append({
                                        'cookie': item,
                                        'token': token,
                                        'pages': len(test_result.get('pages', []))
                                    })
                                else:
                                    results['errors'].append(f"Cookie valid format but token failed")
                            else:
                                results['errors'].append(f"Cookie valid but can't extract token")
                        else:
                        ...
                        if test_result['valid']:
                            ...
                            results['invalid'].append(f"Token: {test_result.get('error', 'Invalid')[:30]}")
        ...
        table = Table(title="Bulk Extraction Results", show_header=True, header_style="bold magenta")
        table.add_column("Type", style="cyan", width=15)
        table.add_column("Count", style="green", width=10)
        table.add_column("Details", width=30)
        
        total_valid = len(results['valid_cookies']) + len(results['valid_tokens'])
        table.add_row("✓ Valid Cookies", str(len(results['valid_cookies'])), 
                     f"{sum(c['pages'] for c in results['valid_cookies'])} total pages")
        table.add_row("✓ Valid Tokens", str(len(results['valid_tokens'])),
        ...
        if results['valid_cookies'] or results['valid_tokens']:
            ...
            if save == 'y':
                ...
                for token_data in results['valid_tokens']:
                    ...
                    saved_count += 1
                self.print_panel("Success", f"Saved {saved_count} items to resources!", "green")
        
        if results['invalid'] or results['errors']:
            show_errors = input("\n[›] Show errors? (y/n): ").lower()
            if show_errors == 'y':
                error_list = "\n".join(results['invalid'][:10] + results['errors'][:10])
